...
public class OldTree {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (otherArgs.length < 2) {
            System.err.println("Usage: OldTree <in> [<in>...] <out>");
            System.exit(2);
        }
        Job job = Job.getInstance(conf, "OldTree");
        job.setJarByClass(OldTree.class);
        job.setMapperClass(OldTreeMapper.class);
        job.setCombinerClass(OldTreeReducer.class);
